...
+ (NSString *)defaultUserAgentString
{
	...
	deviceName = deviceModel? deviceModel : [device model];
	OSName = systemName? systemName : [device systemName];
	OSVersion = systemVersion? systemVersion : [device systemVersion];
	
	if(!deviceModel) deviceModel = [deviceName retain];
	if(!systemName) systemName = [OSName retain];
	if(!systemVersion) systemVersion = [OSVersion retain];
